'use strict';
// Central configuration — everything from environment variables (see .env.example).
...
const baseUrl = str(process.env.BASE_URL, `http://localhost:${int(process.env.PORT, 3001)}`)
  .replace(/\/+$/, '');
...
module.exports = {
  ...
  turnstile: {
    ...
    get enabled() { return Boolean(this.siteKey && this.secretKey); },
  },

  mail: {
    resendKey: str(process.env.RESEND_API_KEY),
    from: str(process.env.MAIL_FROM, 'Secret Santa <no-reply@example.com>'),
    get enabled() { return Boolean(this.resendKey); },
  },

  purgeDays: int(process.env.PURGE_DAYS, 30),

  caps: {
    activeEventsPerOrg: int(process.env.MAX_ACTIVE_EVENTS_PER_ORG, 20),
    membersPerEvent: int(process.env.MAX_MEMBERS_PER_EVENT, 100),
    eventsPerDay: int(process.env.MAX_EVENTS_PER_DAY, 10),
    signupsPerDay: int(process.env.MAX_SIGNUPS_PER_DAY, 20),
    globalActiveEvents: int(process.env.GLOBAL_ACTIVE_EVENT_CAP, 200),
    wishlistItemsPerMember: int(process.env.MAX_WISHLIST_ITEMS, 30),
    editsPerHour: int(process.env.MAX_EDITS_PER_HOUR, 60),
  },
